...
Switch the interface between German and English. Use the slider to set a font size from 11 to 26 px; the choice is saved locally in the browser. The terminal loads `MesloLGM Nerd Font Mono` from Home Assistant's `/local/fonts/` path (or `/local/`); place `MesloLGMNerdFontMono-Regular.ttf` and `MesloLGMNerdFontMono-Bold.ttf` under `/config/www/fonts/` or directly under `/config/www/`. Clients do not need to install the font. When the ATLAS host has Oh My Posh and Bash installed, select a theme from the dropdown for local terminal sessions. The theme list comes from the UGSo fork and the selection is saved in this browser. SSH sessions use the remote server's own shell and do not apply a local theme. ANSI colors are rendered by xterm.js.

The access-token settings are collapsible. They close when a session connects and reopen after disconnecting or when the session ends. If no token is saved in this browser, the settings are shown on initial load. Local shell sessions start at `/`, the filesystem root inside the ATLAS container, rather than `/app`; SSH sessions keep the remote shell's default directory.
