...
- ATLAS framework and Home Assistant package now identify as `0.2.0-alpha.75`.
- Home Assistant App/Add-on package `0.1.207` enables the Supervisor API for
  the opt-in terminal.
...
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.92` for
  repository update detection.
...
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.89` for
  repository update detection.
- Home Assistant App/Add-on package `0.1.199` widens the Card Editor palette
  sidebar again and restores a two-column palette card layout so mapped HACS
  cards no longer overlap or appear squeezed.
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.88` for
  repository update detection.
...
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.86` for
  repository update detection.
- Home Assistant App/Add-on package `0.1.196` makes the Expert Card Editor
  palette panel taller and slightly wider so card entries have more room.
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.85` for
  repository update detection.
...
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.72` for
  repository update detection.
...
- Home Assistant App/Add-on package `0.1.176` keeps the Automation Exporter /
  ...
  or action fragments.
- ATLAS Automation Exporter / Editor now identifies as `0.1.23` for repository
  update detection.
...
- Home Assistant App/Add-on package `0.1.168` removes Automation Exporter /
  ...
  and a `bereinigte-import-version` without `id`.
- ATLAS Automation Exporter / Editor now identifies as `0.1.16` for repository
  update detection.
...
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.59` for
  repository update detection.
- Home Assistant App/Add-on package `0.1.159` keeps direct children of
  horizontal Expert Card Editor stack previews in fixed side-by-side columns.
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.58` for
  repository update detection.
...
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.56` for
  repository update detection.
- Home Assistant App/Add-on package `0.1.156` adds a temporary Card Editor
  loading notice and lets the Expert Card Editor surface grow to fit large
  imported auto-height container cards.
- ATLAS Home Assistant Card Editor now identifies as `0.2.0-alpha.55` for
  repository update detection.
...
- ATLAS Automation Exporter / Editor now identifies as `0.1.14` for repository
  update detection.
- Home Assistant App/Add-on package `0.1.145` stores Automation Exporter /
  Editor exports in timestamped run folders while keeping the YAML filenames
  clean.
- ATLAS Automation Exporter / Editor now identifies as `0.1.13` for repository
  update detection.
- Home Assistant App/Add-on package `0.1.144` makes the Automation Exporter /
  Editor top area calmer by placing compact Status, Source and Export folder
  columns above a single tools action stack.
- ATLAS Automation Exporter / Editor now identifies as `0.1.12` for repository
  update detection.
...
- ATLAS File Studio now identifies as `0.1.29` for repository update detection.
- Home Assistant App/Add-on package `0.1.90` now includes the File Studio
  32x32 icon button sizing update.
- ATLAS File Studio now identifies as `0.1.28` for repository update detection.
...
- Atlas Framework packages now identify the current local preview as
  `0.2.0-alpha.35`, the Home Assistant app package as `0.1.77`, and ATLAS File
  Studio as `0.1.19`.
- File Studio image preview modals are now resizable, remember their size and
  reuse the Card Editor diagonal resize handle style.
- Atlas Framework packages now identify the current local preview as
...
- Atlas Framework packages now identify the current local preview as
  `0.2.0-alpha.17`, and the Home Assistant app package as `0.1.54`.
...
- German Admin, Card Editor and Plugin Hub UI strings now use proper umlauts in
  visible labels, hints and status messages.
...
- Home Assistant status panels now map local entity states, support panel
  registries and validate future connection configuration without connecting.
...
- Theme can now mount and style ATLAS status output through the active Renderer
  DOM-surface route, including safe status replacement on later updates.
...
- Renderer target mount integration readiness catalogs now provide snapshots,
  exports, focused handoff filters and a compact dashboard status summary.
...
* Renderer target mount batch diagnostic catalogs now provide compact snapshots
  and export envelopes with ready/exportable state.
...
* Renderer concrete integration boundary execution closure snapshots now expose
  compact non-executable closure summaries without preparation payloads.
* Renderer concrete integration boundary execution closure snapshot catalogs now
  group compact closure summaries while keeping executable counts fixed at zero.
...
* Renderer concrete integration boundary execution exports now wrap delivery
  snapshot catalogs in non-executable export envelopes.
* Renderer concrete integration boundary execution export snapshots now expose
  compact non-executable export summaries without catalog payloads.
* Renderer concrete integration boundary execution export snapshot catalogs now
  group compact export summaries while keeping executable counts fixed at zero.
* Renderer concrete integration boundary finalizations now wrap execution export
  snapshot catalogs as finalized, non-executable boundary state.
* Renderer concrete integration boundary finalization snapshots now expose
  compact finalized, non-executable summaries without catalog payloads.
* Renderer concrete integration boundary finalization snapshot catalogs now
  group compact finalization summaries while keeping executable counts fixed at
  zero.
...
* Renderer concrete integration boundary finalization handoff snapshots now
  expose compact finalized, non-executable handoff summaries without catalog
  payloads.
* Renderer concrete integration boundary finalization handoff snapshot catalogs
  now group compact handoff summaries while keeping executable counts fixed at
  zero.
...
* Renderer concrete integration boundary finalization export snapshots now
  ...
  payloads.
* Renderer concrete integration boundary finalization export snapshot catalogs
  now group compact export summaries while keeping executable counts fixed at
  zero.
...
* Renderer concrete integration boundary release exports now wrap release
  snapshot catalogs in exported, non-executable envelopes.
* Renderer concrete integration boundary release export snapshots now expose
  compact exported, non-executable export summaries without catalog payloads.
* Renderer concrete integration boundary release export snapshot catalogs now
  group compact export summaries while keeping executable counts fixed at zero.
* Renderer concrete integration boundary release closures now wrap release
  export snapshot catalogs in closed, non-executable closure envelopes.
* Renderer concrete integration boundary release closure snapshots now expose
  compact closed, non-executable closure summaries without catalog payloads.
* Renderer concrete integration boundary release closure snapshot catalogs now
  group compact closure summaries while keeping executable counts fixed at zero.
* Renderer concrete integration boundary release closure exports now wrap
  closure snapshot catalogs in exported, non-executable envelopes.
* Renderer concrete integration boundary release closure export snapshots now
  expose compact exported, non-executable export summaries without catalog
  payloads.
* Renderer concrete integration boundary release closure export snapshot
  catalogs now group compact export summaries while keeping executable counts
  fixed at zero.
* Renderer concrete integration boundary release closure deliveries now wrap
  closure export snapshot catalogs in delivered, non-executable envelopes.
